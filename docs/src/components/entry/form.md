...
| `id` | `string` | - | 表单唯一标识，用于字段设置的本地存储。 |
| `useFieldSettings` | `boolean` | `true` | 是否显示字段设置按钮。 |
...
SunnyForm 提供了强大的字段设置功能，允许用户自定义字段的显示顺序和显隐状态。

### 功能特性

- **字段顺序调整**：通过拖拽字段标题列调整字段显示顺序
- **字段显隐控制**：通过下拉选择控制字段是否显示
- **必填字段保护**：必填字段的显隐状态不可编辑
- **本地存储**：字段设置会自动保存到 localStorage，下次打开时自动应用
- **控制按钮显示**：当鼠标悬停在表单上时，会显示全屏按钮和字段设置按钮，与虚线框的显示逻辑保持一致

### 使用方法

1. **添加表单唯一标识**：在 `useSunnyForm` 配置中添加 `id` 参数

```typescript
const [Form, formApi] = useSunnyForm({
  id: 'd925795d-58c0-4adf-b287-cc04233ada6d', // 唯一标识
  schema: [...],
  // 其他配置
});
```

2. **打开字段设置**：当鼠标悬停在表单上时，点击表单右上角的字段设置按钮
...
4. **保存设置**：点击保存按钮，设置会自动保存到 localStorage

### 全屏功能

SunnyForm 支持表单全屏显示，点击表单右上角的全屏按钮即可切换全屏状态。

**特性**：
- 全屏状态下，表单会占据整个屏幕空间，提供更大的操作区域
- 全屏状态下，控制按钮会始终显示，不会随鼠标移动而隐藏
- 全屏状态下，按 Esc 键不会关闭包含表单的弹窗
- 非全屏状态下，按 Esc 键可以关闭包含表单的弹窗

### 技术实现

- **存储机制**：使用 localStorage 存储字段设置，键名为 `form-field-settings-{formId}`
- **初始化逻辑**：表单初始化时会自动读取 localStorage 中的设置
- **状态管理**：通过更新 schema 的 `hidden` 属性控制字段显隐

### 注意事项

- **必填字段**：必填字段的显隐状态不可修改，始终保持显示
- **存储限制**：localStorage 有存储容量限制，适用于中小规模表单
- **跨浏览器**：设置只在当前浏览器生效，不同浏览器间不共享
...
// 不推荐：使用动态生成的 ID
const [Form, formApi] = useSunnyForm({
  id: `form-${Date.now()}`, // 每次刷新都会生成新 ID，设置无法持久化
  schema: userSchema,
});
```
...
- **核心字段**：将重要字段放在前面
- **逻辑分组**：按照业务逻辑组织字段顺序
- **默认显示**：只显示最常用的字段，其他字段可通过字段设置控制显示

### 3. 结合字段联动使用

字段设置与字段联动可以结合使用，实现更灵活的表单控制：
...
这样，用户可以通过字段设置调整字段顺序，同时通过字段联动控制条件显示的字段。

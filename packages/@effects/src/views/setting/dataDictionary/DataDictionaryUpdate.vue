...
defineOptions({
  name: 'DataDictionaryUpdate'
})

interface Props {
  visible: boolean
  row?: DataDictionaryVO
}
...
watch(() => props.visible, async (val) => {
  if (val && props.row) {
    ...
    try {
      // 调用修改初始化接口
      const res = await requestClient.post<{ result: DataDictionaryVO }>('/core/authDict/editInit', {
        authDict: {
          id: props.row.id
        }
      })
...
async function handleSubmit() {
  try {
    ...
    const res = await requestClient.post<{ message?: string }>('/core/authDict/edit', params)

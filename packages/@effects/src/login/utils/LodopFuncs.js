...
export function needCLodop() {
  try {
    ...
    if (RegExp(/Windows\sPhone/i).exec(ua) != null) return true
    if (RegExp(/iPhone|iPod/i).exec(ua) != null) return true
    if (RegExp(/Android/i).exec(ua) != null) return true
    if (RegExp(/Edge\D?\d+/i).exec(ua) != null) return true

    var verTrident = RegExp(/Trident\D?\d+/i).exec(ua)
    var verIE = RegExp(/MSIE\D?\d+/i).exec(ua)
    var verOPR = RegExp(/OPR\D?\d+/i).exec(ua)
    var verFF = RegExp(/Firefox\D?\d+/i).exec(ua)
    var x64 = RegExp(/x64/i).exec(ua)
    if ((verTrident == null) && (verIE == null) && (x64 !== null)) { return true } else
    if (verFF !== null) {
      verFF = RegExp(/\d+/).exec(verFF[0])
      if ((verFF[0] >= 42) || (x64 !== null)) return true
    } else
    if (verOPR !== null) {
      verOPR = RegExp(/\d+/).exec(verOPR[0])
      if (verOPR[0] >= 32) return true
    } else
    if ((verTrident == null) && (verIE == null)) {
      var verChrome = RegExp(/Chrome\D?\d+/i).exec(ua)
      if (verChrome !== null) {
        verChrome = RegExp(/\d+/).exec(verChrome[0])

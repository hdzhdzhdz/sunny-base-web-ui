<template>
  <a-dropdown trigger="click" @select="handleSelect">
    ...
    >
      <KunkkaIcon :icon="currentIcon" :size="20" />
    </div>
    <template #content>
      <a-doption value="light">
        <template #icon><KunkkaIcon icon="lucide:sun" /></template>
        <span>浅色模式</span>
        <template #suffix v-if="mode === 'light'"><KunkkaIcon icon="lucide:check" class="text-[rgb(var(--primary-6))]" /></template>
      </a-doption>
      <a-doption value="dark">
        <template #icon><KunkkaIcon icon="lucide:moon" /></template>
        <span>暗色模式</span>
        <template #suffix v-if="mode === 'dark'"><KunkkaIcon icon="lucide:check" class="text-[rgb(var(--primary-6))]" /></template>
      </a-doption>
      <a-doption value="auto">
        <template #icon><KunkkaIcon icon="lucide:monitor" /></template>
        <span>跟随系统</span>
        <template #suffix v-if="mode === 'auto'"><KunkkaIcon icon="lucide:check" class="text-[rgb(var(--primary-6))]" /></template>
      </a-doption>
    </template>
  </a-dropdown>
</template>

...
/**
 * Menu 组件
 * - 支持垂直/水平两种模式，暗色/亮色两种主题
 * - 提供折叠、手风琴、滚动到激活项等功能
 * - 通过上下文在子菜单与菜单项之间共享状态与行为
 */
...
/**
 * 组件属性
 * - accordion: 是否开启手风琴模式（同级只展开一个）
 * - collapse: 是否折叠菜单（仅显示图标）
 * - mode: 菜单模式（'vertical' | 'horizontal'）
 * - rounded: 是否圆角风格
 * - theme: 主题（'dark' | 'light'）
 * - defaultActive: 默认激活路径
 * - defaultOpeneds: 默认展开的子菜单路径集合
 * - scrollToActive: 激活后是否滚动到该项
 */
...
/**
 * 向后代组件提供菜单上下文：
 * - 状态：activePath、openedMenus、items、subMenus、theme 等
 * - 行为：openMenu/closeMenu、点击事件处理、注册/移除项等
 */
createMenuContext(
  reactive({
    activePath,
